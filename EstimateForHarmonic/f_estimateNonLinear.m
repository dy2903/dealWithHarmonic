...
% # ************ get the point with misError************
	...
	% for l = 1 : order + 1
		% for m = 1 : NumChannel 
			% decimaFre= -(l - 1) * fin + ( m - 1) * fs_adc / M ;
			% if decimaFre <  0 
				% fre2Point = (N / fs_adc) * (decimaFre + fs_adc);
				% t_m (l , m) = dataAfterFFT (fix(fre2Point) + 1);
				% % t_m (l , m) = dataAfterFFT (fix( (N / fs_adc ) * (decimaFre + fs_adc / N + fs_adc)) ) ;
			% else
				% % fre2Point = (N / fs_adc) * decimaFre;
				% % t_m (l , m) = dataAfterFFT (fix( (N / fs_adc ) * ( decimaFre + fs_adc / N) ));
				% fre2Point = (N / fs_adc) * (decimaFre);
				% t_m (l , m) = dataAfterFFT (fix(fre2Point) + 1);
				% end
		% end
